...
import { isGitInternalPath, resolveSafePath } from '../src/security/path.js';
...
test('character replacement uses one-based Unicode columns and supports cross-line ranges', () => {
  assert.equal(
    replaceCharacters('A😀B\nnext', { line: 1, column: 2 }, { line: 1, column: 3 }, 'X'),
    'AXB\nnext'
  );
  assert.equal(
    replaceCharacters('ab\ncd', { line: 1, column: 2 }, { line: 2, column: 2 }, 'X'),
    'aXd'
  );
  assert.throws(
    () => replaceCharacters('abc', { line: 1, column: 5 }, { line: 1, column: 5 }, ''),
    /column must be between/
  );
  assert.throws(
    () => replaceCharacters('a\nb', { line: 2, column: 2 }, { line: 1, column: 1 }, ''),
...
test('registered read_file and edit_file tools read and edit existing files', async () => {
  await withTemporaryDirectory(async (root) => {
    await writeFile(path.join(root, 'sample.txt'), 'alpha\nbeta\n');
    const { server, tools } = captureTools();
    registerFileTools(server, { root });

    assert.deepEqual([...tools.keys()], ['list_directory', 'read_file', 'edit_file']);
    ...
    assert.equal(editResult.structuredContent.changed, true);
    assert.equal(await readFile(path.join(root, 'sample.txt'), 'utf8'), 'alpha\nBETA\n');

    const invalidResult = await tools.get('edit_file').callback({
      path: 'sample.txt',
      operation: 'replace_characters',
