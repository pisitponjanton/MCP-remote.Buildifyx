...
function createRemoteDashboardState(instance, initialDashboard) {
  ...
  async function rpc(command, payload = {}, timeoutMs = 1500) {
    const response = await requestInstanceControl(instance, command, { timeoutMs, payload });
...
export async function runAttachedDashboard(instance) {
