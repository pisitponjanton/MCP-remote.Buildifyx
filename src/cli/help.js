...
Each instanceId owns its own Permissions, additional allowed roots, and command rules, even when multiple instances use the same workspace path. New instances start from defaults. Foreground Ctrl+C removes the running registry record but does not reset the policy unless the instance is explicitly removed. Foreground → background handoff and bdxa attach keep the same instanceId and therefore preserve that instance's settings. Cloud remains the default mode and requires device login. Local MCP uses No Auth, listens only on 127.0.0.1, and keeps its state isolated under ~/.buildifyx/local. Expose the Local port through your own HTTPS tunnel/reverse proxy when a remote MCP client needs access. Treat the exposed URL as a secret because anyone who can reach it can invoke tools under the workspace permissions.
`);
}
