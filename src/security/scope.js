...
import { isBuildifyxInternalPath, isInsideRoot } from './path.js';

function matchingRoot(roots, target) {
  return roots.find((root) => isInsideRoot(root, target)) ?? null;
}

function assertNotProtectedPath(target) {
